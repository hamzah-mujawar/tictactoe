...
char makeTurn(char gameBoard[Gameparams::boardSize][Gameparams::boardSize],
              char currentPlayer)
{
    int placeRow{};
    int placeCol{};
...
bool checkRows(char gameBoard[Gameparams::boardSize][Gameparams::boardSize],
               char player)
{
    for (int i = 0; i < Gameparams::boardSize; i++)
    {
        if (gameBoard[i][0] == player && gameBoard[i][1] == player &&
...
bool checkCols(char gameBoard[Gameparams::boardSize][Gameparams::boardSize],
               char player)
{
    for (int i = 0; i < Gameparams::boardSize; i++)
    {
        if (gameBoard[0][i] == player && gameBoard[1][i] == player &&
...
bool checkDiags(char gameBoard[Gameparams::boardSize][Gameparams::boardSize],
                char player)
{
    if (gameBoard[0][0] == player && gameBoard[1][1] == player &&
        gameBoard[2][2] == player)
        return true;
    else if (gameBoard[2][0] == player && gameBoard[1][1] == player &&
             gameBoard[0][2] == player)
        return true;
    return false;
}

bool checkWin(char gameBoard[Gameparams::boardSize][Gameparams::boardSize],
              char player)
{
    return (checkRows(gameBoard, player) || checkCols(gameBoard, player) ||
            checkDiags(gameBoard, player));
}
...
int main()
{
    ...
    char currentPlayer{
        Gameparams::player1}; // setting player 1 to be X (game starts with X)
    ...
    while (!checkWinner)
    {
        ...
        currentPlayer = makeTurn(
            gameBoard, currentPlayer); // this function returns the next player
